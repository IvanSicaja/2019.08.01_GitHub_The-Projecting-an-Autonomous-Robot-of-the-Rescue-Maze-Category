...
void ROBOT_GIBANJE::Go_Forward_30cm(int motor_speed ){ // Default speed is maximum when you don't send any value to function
        ...
        while(Read_Forward_Middle_Distance() > (first_distance - 300 + allowed_error)) { // -300 da ide za 30 cm unaprijed
...
void ROBOT_GIBANJE::Go_180degrees(int motor_speed ){ // Default speed for turning is 30, when you don't send any value to function
...
bool ROBOT_GIBANJE::Is_Wall_Left(){
         if ((Read_Left_Back_Distance() < Check_Walls_Distance) || (Read_Left_Front_Distance() < Check_Walls_Distance)) return true;
        else return false;
}
bool ROBOT_GIBANJE::Is_Wall_Right(){
        if ((Read_Right_Back_Distance() < Check_Walls_Distance) || (Read_Right_Front_Distance() < Check_Walls_Distance)) return true;
        else return false;
}
bool ROBOT_GIBANJE::Is_Wall_Forward(){
        if ((Read_Forward_Left_Distance() < Check_Walls_Distance) || (Read_Forward_Middle_Distance() < Check_Walls_Distance) || (Read_Forward_Right_Distance() < Check_Walls_Distance)) return true;
        else return false;
}
bool ROBOT_GIBANJE::Is_Wall_Back(){
        if ((Read_Back_Left_Distance() < Check_Walls_Distance) || (Read_Back_Middle_Distance() < Check_Walls_Distance) || (Read_Back_Right_Distance() < Check_Walls_Distance)) return true;
        else return false;
}
...
bool ROBOT_GIBANJE::Moveing_Is_Wall_Left(){
               if ((Read_Left_Back_Distance() < Stop_Distance) && (Read_Left_Front_Distance() < Stop_Distance)) return true;
        else return false;
}
bool ROBOT_GIBANJE::Moveing_Is_Wall_Right(){
         if ((Read_Right_Back_Distance() < Stop_Distance) &&(Read_Right_Front_Distance() < Stop_Distance)) return true;
        else return false;
}
bool ROBOT_GIBANJE::Moveing_Is_Wall_Forward(){
         if ((Read_Forward_Left_Distance() < Stop_Distance) && (Read_Forward_Middle_Distance() < Stop_Distance) && (Read_Forward_Right_Distance() < Stop_Distance)) return true;
        else return false;
}
bool ROBOT_GIBANJE::Moveing_Is_Wall_Back(){
            if ((Read_Back_Left_Distance() < Stop_Distance) && (Read_Back_Middle_Distance() < Stop_Distance) && (Read_Back_Right_Distance() < Stop_Distance)) return true;
        else return false;
}
...
uint16_t Alowed_Error= 10; //This two sensors values need to be very closed because we want precise robot alignment toward left wall
