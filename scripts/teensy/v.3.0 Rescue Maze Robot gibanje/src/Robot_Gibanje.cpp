...
{
        if ((Read_Forward_Left_Distance() < Stop_Distance) && (Read_Forward_Middle_Distance() < Stop_Distance) && (Read_Forward_Right_Distance() < Stop_Distance))
                return true;
        else
                return false;
}
...
{
        if ((Read_Back_Left_Distance() < Stop_Distance) && (Read_Back_Middle_Distance() < Stop_Distance) && (Read_Back_Right_Distance() < Stop_Distance))
                return true;
        else
                return false;
}
...
void ROBOT_GIBANJE::Center_Robot()
{
        ...
        if ((Read_Forward_Left_Distance() < (Stop_Distance - 10)) && (Read_Forward_Right_Distance() < (Stop_Distance - 10)))
        {
                while ((Read_Forward_Left_Distance() < (Stop_Distance - 10)) && (Read_Forward_Right_Distance() < (Stop_Distance - 10)))
                {
                        Go_Back();
        ...
        if ((Read_Forward_Left_Distance() > (Stop_Distance + 20)) && (Read_Forward_Right_Distance() > (Stop_Distance + 20)))
        {
                while ((Read_Forward_Left_Distance() > (Stop_Distance + 20)) && (Read_Forward_Right_Distance() > (Stop_Distance + 20)))
                {
                        Go_Forward();
                        Read_Forward_Left_Distance();
                        Read_Forward_Right_Distance();
...
void ROBOT_GIBANJE::Automomus_Maze_Path_Finding()
{
        ...
        if ((Read_Forward_Left_Distance() > 500) && (Read_Forward_Right_Distance() > 500))
        {
                Go_Forward_30cm();
        ...
        else if ((Read_Forward_Left_Distance() < 500) && (Read_Forward_Right_Distance() < 500))
        {
                Go_Forward();
                Serial.println("Going forward to the first wall.");
                Serial.print("Autonomus mode-Forward Left Distance:");
