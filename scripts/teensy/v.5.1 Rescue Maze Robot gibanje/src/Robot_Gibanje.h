...
#define FORWARD2_AN_PIN 34 // Forward Right LIDAR - AN pin
#define LEFT1_AN_PIN 35    // Left Front LIDAR - AN pin
#define LEFT2_AN_PIN 36    // Left Back LIDAR - AN pin
#define RIGHT1_AN_PIN 37   // Right Front LIDAR - AN pin
#define RIGHT2_AN_PIN 38   // Right Back LIDAR - AN pin

#define LEFT_THERMAL_CAM_OUT_PIN 23 // Thermal sensor - OUT pin
...
public:
    ...
    float Read_Heading();
    float Read_Gyroscope_Z_Orientation();
    void Read_Temperature_Left_Sensor();
    ...
    bool Stopped_Is_Wall_Left_AND_Logic();    //OR logic operator used
    bool Stopped_Is_Wall_Right_AND_Logic();   //OR logic operator used
    bool Stopped_Is_Wall_Forward_AND_Logic(); //OR logic operator used

    bool Moveing_Is_Wall_Forward_OR_Logic();  //AND logic operator used
    ...
    void Keep_Optimal_Distance_With_Left_Wall(int Ideal_Left_Distance = 13); // Not  Deklared
    void Keep_Optimal_Distance_With_Right_Wall(int Ideal_Right_Distance = 13); // Need Optimization

    void Stay_Away_From_Left_Wall();
    void Stay_Away_From_Right_Wall();

    int Read_Right_Encoder(int right_motor_speed = 20);
    int Read_Left_Encoder(int left_motor_speed = 20);

    void Find_and_Rotate_To_Opend_Wall(int opendWallsDIstance = 30, int delayInMicroseconds = 1000); //Successed test

    // Color sensor functions
    int Search_for_Black_or_Grey_tiles();
    int getRedPW();
    int getGreenPW();
    int getBluePW();

    void Go_Forward_PID_Corrected(int Setpoint1, int Orientation_Tolerance = 1, int delayTime = 0);
    int Optimize_speed_with_Z_Orientation(int Z_Orientatio_Tolerance = 13);
    void Go_Autonomus_with_PID(int Orientation_Tolerance = 1, int delayTime = 0);
    void Full_Autonomus_Function(int Orientation_Tolerance = 1, int delayTime = 0);
};
