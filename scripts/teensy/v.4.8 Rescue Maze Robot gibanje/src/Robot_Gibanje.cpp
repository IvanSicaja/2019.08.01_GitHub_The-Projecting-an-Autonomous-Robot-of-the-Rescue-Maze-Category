...
void ROBOT_GIBANJE::SetupZaSveKomponente()
{
        ...
        pinMode(LEFT_THERMAL_CAM_OUT_PIN, INPUT); // Analog input pin 50, connected to thermal sensor OUT output
...
{
        if ((Read_Left_Front_Distance() > 250))
                return true;
        else
                return false;
}
...
{
        if ((Read_Right_Front_Distance() > 250))
                return true;
        else
                return false;
}
...
int ROBOT_GIBANJE::Optimize_speed_with_Z_Orinetetion(int Z_Orientatio_Tolerance = 13)
{
        int left_and_right_motor_speed;
        int Z_Orientation = Read_Gyroscope_Z_Orientation();

        Z_Orientation = Read_Gyroscope_Z_Orientation();
...
void ROBOT_GIBANJE::Read_Temperature_Left_Sensor()
{
        ...
        while (outputState > 400)
        {
                Serial.println(outputState);
                Serial.println("Temperature victim recognised!");
                Stop_Robot();
                delay(300); 
                outputState = analogRead(LEFT_THERMAL_CAM_OUT_PIN);
        }
}
...
void ROBOT_GIBANJE::Go_Autonomus_with_PID(int Orientation_Tolerance = 1, int delayTime = 0)
{
        ...
        left_and_right_motor_speed = Optimize_speed_with_Z_Orinetetion();
        ...
        //Prevous while condition
        //(time_2 - time_1) < 40000
        //(Forward_Left_distance > 200) && (Forward_Right_distance > 200)
        ...
        while ((Forward_Left_distance > 200) && (Forward_Right_distance > 200) && (founded_black_or_gray_tiles == false)) //condition - probably encoder distance
        {
                ...
                */
                ...
                while ((Left_Front_Distance < 240) && (Forward_Left_distance > 200) && (Forward_Right_distance > 200) && (founded_black_or_gray_tiles == false))
                {
                        Serial.println(" ");
                        Serial.println("GO RIGHT - Too closed to Leftt wall. #########################################################");
                        Serial.println(" ");
                ...
                while ((Right_Front_Distance < 200) && (Forward_Left_distance > 200) && (Forward_Right_distance > 200) && (founded_black_or_gray_tiles == false))
                {
                        Serial.println(" ");
