...
uint16_t ROBOT_GIBANJE::Read_Forward1_Distance(){
        return lidar.distance(0);
}

uint16_t ROBOT_GIBANJE::Read_Forward2_Distance(){
        return lidar.distance(1);
}

uint16_t ROBOT_GIBANJE::Read_Forward3_Distance(){
...
uint16_t ROBOT_GIBANJE::Read_Back1_Distance(){
        return lidar.distance(7);
}

uint16_t ROBOT_GIBANJE::Read_Back2_Distance(){
        return lidar.distance(8);
}

uint16_t ROBOT_GIBANJE::Read_Back3_Distance(){
...
void ROBOT_GIBANJE::Go_Forward_30cm(int speed_and_direction ){ // Default speed is maximum when you don't send any value to function
        ...
        uint16_t first_distance = Read_Forward2_Distance();
        ...
        while(Read_Forward2_Distance() > (first_distance - 300 + allowed_error)) { // -300 da ide za 30 cm unaprijed
...
void ROBOT_GIBANJE::Go_Back_30cm(int speed_and_direction ){
        ...
        while(Read_Forward2_Distance() < (first_distance + 300 - allowed_error)) {
...
bool ROBOT_GIBANJE::Is_Wall_Forward(){
        if(Read_Forward2_Distance()<300) return true;
        else return false;
}
bool ROBOT_GIBANJE::Is_Wall_Back(){
        if(Read_Back2_Distance()<300) return true;
        else return false;
}
