...
class Player(turtle.Turtle):
    ...
    def go_down(self):
        ...
        print('Trenutne coordinate robota, X=',player.xcor()," Y=",player.ycor(),".")
...
def setup_maze(level):
    for y in range(len(level)):
        for x in range(len(level[y])):
            character=level[y][x]
            #Calculate the screen x, y coordinstes
            screen_x=-432 + (x*24)
            screen_y=312 - (y*24)
            ...
            if character=="X":
                ...
                pen.stamp()                                #Add pen on screen and return pen's id
            ...
            '''
            #If signal is for "RED" victim, draw it to the screen
            if character=="R":
                treasure.append(RedVictim(screen_x,screen_y))
            
             #If signal is for "ORANGE" victim, draw it to the screen
            if character=="O":
                treasure.append(OrangeVictim(screen_x,screen_y))
            
             #If signal is for "GREEN" victim, draw it to the screen
            if character=="G":
                treasure.append(GreenVictim(screen_x,screen_y))
            '''
#Create class instances

#pen=Pen()                    #For Pen in this purpose isn't needed
player=Player() 
